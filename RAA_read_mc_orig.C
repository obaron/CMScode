...
void RAA_read_mc_orig(char *algo = "Pu"){
...
/*   
     const int nbinsPP_pthat = 11; //THIS SET IS VESTIGIAL, DO NOT USE
  ...
  */
  ...
  for(int k = 0;k<no_radius;k++){
	...
	//hpbpb_response[h] = new TH2F(Form("hpbpb_response_cent%d",i),Form("response jtpt refpt %2.0f - %2.0f cent",5*boundaries_cent[h],5*boundaries_cent[i+1]),nbins_pt,boundaries_pt,nbins_pt,boundaries_pt);
	  ...
	  hpp_etadist[k] = new TH1F(Form("hpp_eta_R%d",list_radius[k]),Form("eta dist R%d",list_radius[k]),500,-4,4);
	  hpp_phidist[k] = new TH1F(Form("hpp_phi_R%d",list_radius[k]),Form("phi dist R%d",list_radius[k]),500,-1*TMath::Pi(),TMath::Pi());
...
/*			DEBUG
			cout<<"going to fill pp eta dist with subEvt info for kicks"<<endl;
			hpp_etadist[k][j]->Fill(subEvt);
			cout<<"going to fill pp phi dist with subEvt info for kicks"<<endl;
			hpp_phidist[k][j]->Fill(subEvt);
			cout<<"hey, you got this far, what's the variables we're trying to fill?"<<endl;
			cout<<"h is: "<<h<<", g is: "<<g<<", j is: "<<j<<", k is: "<<k<<endl;
			cout<<"This event has "<<dataPP[k][h]->njets<<" jets."<<endl;
			cout<<"dataPP[k][h]->jteta[g]: "<<dataPP[k][h]->jteta[g]<<endl;
			cout<<"dataPP[k][h]->jtphi[g]: "<<dataPP[k][h]->jtphi[g]<<endl;
*/			
  ...
  TFile f(Form("/net/hisrv0001/home/obaron/CMSSW_5_3_16/drawfiles/output/PbPb_pp_mc_ak%s_%d.root",algo,date.GetDate()),"RECREATE");
  f.cd();
