//MIT VERSION UPDATED APRIL 21 2015
#include <iostream>
#include <stdio.h>
#include <fstream>
#include <fstream>
...
// new plots
 ...
 fout.cd();
  ...
  for(int i = 0; i<nbins_cent;++i){ //Write centrality histograms
	...
	cmatrix_HTL[i]->SaveAs(Form("hpbpb_MCmatrix_HLT_R%d_n20_eta_p20_cent%d.pdf",radius,i),"RECREATE");
	...
	cPbPb_pteta_noCut[i]->SaveAs(Form("hData_pTeta_noCut_R%d_n20_eta_p20_cent%d.pdf",radius,i),"RECREATE");
	...
	cPbPb_pteta_CutA[i]->SaveAs(Form("hData_pTeta_CutA_R%d_n20_eta_p20_cent%d.pdf",radius,i),"RECREATE");
	...
	cMC_pteta_noCut[i]->SaveAs(Form("hMC_pTeta_noCut_R%d_n20_eta_p20_cent%d.pdf",radius,i),"RECREATE");
	cMC_pteta_CutA[i]->cd();
	...
	cMC_pteta_CutA[i]->SaveAs(Form("hMC_pTeta_CutA_R%d_n20_eta_p20_cent%d.pdf",radius,i),"RECREATE");
...
//WRITE Phi vs Eta dist.
 ...
 cMC_noCut_phi_eta->SaveAs(Form("MC_noCut_phi_eta_R%d_n20_eta_p20.pdf",radius),"RECREATE");
 
 TCanvas * cMC_CutA_phi_eta = new TCanvas("cMC_noCut_phi_eta","",1000,800);
 cMC_CutA_phi_eta->cd();
 hMC_CutA_phi_eta->Write(); 
 hMC_CutA_phi_eta->Draw("colz"); 
 cMC_CutA_phi_eta->SaveAs(Form("MC_CutA_phi_eta_R%d_n20_eta_p20.pdf",radius),"RECREATE");
 ...
 cPbPb_noCut_phi_eta->SaveAs(Form("Data_noCut_phi_eta_R%d_n20_eta_p20.pdf",radius),"RECREATE");
 ...
 cPbPb_CutA_phi_eta->SaveAs(Form("Data_CutA_phi_eta_R%d_n20_eta_p20.pdf",radius),"RECREATE");
...
cJet65_CutEfficiency_Jet65->Close();
  ...
  cTriggerCombination->SaveAs(Form("TriggerCombination_YetkinCuts_R0%d.pdf",radius));
  ...
  for(int i = 0 ; i < nbins_cent ; ++i){
    ...
    cenoCut_phi_cent[i]->SaveAs(Form("NoCuts_R%d_phi_cent%d_MCweight.pdf",radius,i),"RECREATE");
  ...
  for(int i = 0 ; i < nbins_cent ; ++i){
    ...
    ceCutA_eta_cent[i]->SaveAs(Form("CutA_R%d_eta_cent%d_MCweight.pdf",radius,i),"RECREATE");
